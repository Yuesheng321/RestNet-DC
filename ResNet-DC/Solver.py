...
class Solver(object):
    def __init__(self, net, loader, model='train'):
        ...
        if self.model == 'train':
            ...
            self.loss_mask = nn.CrossEntropyLoss(reduction='mean')
            # self.classloss = nn.cross_entropy
    ...
    def train(self):
        ...
        for index, sample in enumerate(self.train_loader):
            ...
            # 预测标签
            pre_denses, pre_masks = self.net(imgs)  # shape[N, 2, H, W]
            # 计算损失
            loss_mask = self.loss_mask(pre_masks, gt_masks)
            loss2 = self.loss2(pre_denses, gt_denses)
            loss = loss_mask + loss2

            loss1_meter.update(loss_mask)
            loss2_meter.update(loss2)
            loss_meter.update(loss)
            print('loss1:{}, loss2:{}, loss:{}'.format(loss_mask.data, loss2.data, loss.data))
            ...
            my_n_cors = localmax(pre_denses, pre_masks)
            # 真实位置和分数
            n_cors = localmax(gt_denses, gt_masks)
    ...
    def train1(self):
        ...
        for index, sample in enumerate(self.train_loader):
            ...
            loss = self.loss2(pre_denses, gt_denses)
        ...
        if self.epoch % self.vis_fre == 0:
            vis.vis(self.writer, pre_denses[0, 0], gt_denses[0, 0], self.epoch, model='train')
    ...
    def val1(self):
        ...
        if self.epoch % self.vis_fre == 0:
            vis.vis(self.writer, pre_denses[0, 0], gt_denses[0, 0], self.epoch, model='val')
    ...
    def train2(self):
        ...
        for index, sample in enumerate(self.train_loader):
            ...
            gt_denses = trans.tovariable(sample['dense']) * self.scale
            # 预测标签
            pre_denses = self.net(imgs)  # shape[N, 2, H, W]
            # 计算损失
            loss_mse = self.loss2(pre_denses, gt_denses)
            loss_ker = myLoss.loss4(pre_denses, gt_denses)
            loss = loss_mse + loss_ker

            loss1_meter.update(loss_mse)
            loss2_meter.update(loss_ker)
            loss_meter.update(loss)
            print('loss1:{}, loss2:{}, loss:{}'.format(loss_mse.data, loss_ker.data, loss.data))
